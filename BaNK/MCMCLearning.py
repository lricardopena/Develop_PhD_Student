...
def splitOneComponent(pi, means, lambdaInverse, A, Z, delta, X, tau, niu, rhoSquare, ellInverse, xi, r):
    ...
    for nk in Z[k]:
        sigmak = np.dot(np.dot(A, np.diag(1.0 / np.array(sorted(newlambdaInversek)))), A)
        pnk = np.log(newPik) + np.log(np.linalg.det(sigmak)) + (-1.0/2 * np.dot(np.dot((X[nk] - newMeank).T, np.linalg.inv(sigmak)),(X[nk] - newMeank)))

        sigmaj = np.dot(np.dot(A, np.diag(1.0 / np.array(sorted(newlambdaInversej)))), A)
        pnj = np.log(newPij) + np.log(np.linalg.det(sigmaj)) + (-1.0 / 2 * np.dot(np.dot((X[nk] - newMeanj).T, np.linalg.inv(sigmaj)), (X[nk] - newMeanj)))
    ...
    secondExponent = 0
    for d in range(0, D):
        secondExponent += newlambdaInversej[d] * ellInversej[d] + newlambdaInversek[d] * ellInversek[d] - lambdaInverse[k][d] * ellInverse[k][d]
