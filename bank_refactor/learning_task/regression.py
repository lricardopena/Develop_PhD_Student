...
from bank_refactor.base_bank import BaseBank, BankLocallyStationary


class BankRegression(BaseBank):
...
class Bank_Locally_Regression(BankLocallyStationary):
    def __compute_log_model_evidence(self, frequences, a0=0.001, b0=0.001, alpha_prior=0.000001, miu0=None):
        ...
        if miu0 is None:
            ...
            bn = b0 + 1. / 2 * (self.Y.dot(self.Y) + miu0.T.dot(invLamba0).dot(miu0) - miun.T.dot(
                invLamban).dot(miun))
        result_log = 1. / 2 * np.linalg.slogdet(Lamdan)[1] + a0 * np.log(b0) + sc.special.gammaln(an)
        result_log += - an * np.log(bn) - sc.special.gammaln(a0) - (self.M1 * self.M2 * 2) / 2. * np.log(
            1. / alpha_prior)
        return result_log
    ...
    def __sample_omega(self):
        ...
        # The complexity of this is O(N d M + M^2 N + M^3)
        actual__log_error = self.__compute_log_model_evidence(self.frequencies)
