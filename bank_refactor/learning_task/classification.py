...
class BankClassification(BaseBank):
...
class BankLocallyClassification(BankLocallyStationary):
    ...
    def predict_proba(self, X):
        alpha_prior = 0.000001
        Phi_x = self.matrix_phi_with_X(self.frequencies, self.X)
        invLamba0 = alpha_prior * np.identity(self.M1 * self.M2 * 2 + self.bias)
        Lamdan = np.linalg.inv(Phi_x.T.dot(Phi_x) + invLamba0)
        miun = Lamdan.dot(Phi_x.T.dot(self.Y))
        Phi_x = self.matrix_phi_with_X(self.frequencies, X)
        result = Phi_x.dot(miun)
        result = 1 / (1 + np.exp(-result))
