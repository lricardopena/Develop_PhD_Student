...
def sample_from_sk(Tk, mean, sigma, sk, x0, xk_plus_1, Z):
    C = np.sum(sk)
    sk = sk / C
    cumulative_sk = np.cumsum(sk)
    new_cumulativeSk = [0]

    size_of_bins = int(np.floor(len(cumulative_sk) / (len(Z) - 1)))

    # Z = np.delete(Z, 0)

    for i in range(1, len(Z)):
        actual_cumulative_sk = cumulative_sk[i * size_of_bins - 1]
        new_cumulativeSk.append(actual_cumulative_sk)
    # for j, prob in enumerate(new_cumulativeSk):
    # if j > 0:
    # plt.plot([j, j], [prob, 0], label='Cumultave corresponding to ' + str(Tk[j-1]))
    # else:
    # plt.plot([j, j], [prob, 0], label='Cumultave corresponding to ' + str(x0))

    cumulative_sk = new_cumulativeSk
    # plt.plot(range(len(cumulative_sk)), cumulative_sk, label='Cumulate')
    u = np.random.uniform()
    for j, probability in enumerate(cumulative_sk):
        if probability >= u:
            # plt.plot([0, j, j], [u, u, 0], label='random number')
            if u > (cumulative_sk[j] - cumulative_sk[j - 1]) / 2.:
                xj = Tk[j - 1]
            else:
                xj = Tk[j - 2]
...
def perform_ARS_log_normal(mean, sigma, number_of_samples, Tk, x0, xk_plus_1, plot_every_step=False):
    x_samples = []
    number_of_elements_linespace = 1000
    while len(x_samples) < number_of_samples:
        ...
        Sk = compute_sk_sample_x(Tk, mean, sigma, x0, xk_plus_1, number_of_elements_linespace)
        x_proposal = sample_from_sk(Tk, mean, sigma, Sk, x0, xk_plus_1, Zk)
...
def main():
    ...
    x_samples = perform_ARS_log_normal(mean, sigma, 100, Tk, x0, xk_plus_1, plot_every_step=False)
