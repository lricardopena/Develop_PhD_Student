...
def updateMixingWeights(delta, Z, X, oldPi, means, lambdaInverse):
    k = len(Z)
    q = np.zeros(k)
    u = np.zeros(k)
    nk = []
...
def updateMeansAndCovariances(r, xi, tau, ellInverse, Z, X, means, lambdaInverse, A, pi):
    ...
    for m in range(0, M):
        if len(Z[m]) > 0:
            ...
            for x in X[Z[m]]:
                Sm += np.dot((x - newMeans[m]).reshape(-1, 1), np.transpose(x - newMeans[m]).reshape(1, -1))

            beta = tau[m] * (newMeans[m] - xi[m]).reshape((-1, 1)) * (newMeans[m] - xi[m]).reshape(
                (1, -1)) + Sm
            for d in range(0, D):
            ...
            # Get the new mean
            cov = np.dot(np.dot(A, np.diag(1.0 / np.array(sorted(newlamdaInverse[m])))), A)
...
def updateHyperparameters(r, zeta, xi, tau, niu, ellInverse, rhoSquare, means, lambdaInverse, A):
    ...
    newXi = np.array(xi)
    newTau = np.array(tau)
    newEllInverse = np.array(ellInverse)
    inverseRhoSquare = 1.0 / rhoSquare

    for m in range(0, M):
        for d in range(0, D):
            newEllInverse[m][d] = gamma.rvs((1 + r[m]) / 2.0, (lambdaInverse[m][d] + zeta[d]) / 2.0, size=1)

        inverseSigmam = np.dot(np.dot(A, np.diag(sorted(lambdaInverse[m]))), A)
        sigmaHat = np.linalg.inv(inverseRhoSquare * np.identity(D) + newTau[m] * inverseSigmam)
        newmean = np.dot(sigmaHat, inverseRhoSquare * niu + newTau[m] * np.dot(inverseSigmam, means[m]))

        try:
            newXi[m] = multivariate_normal.rvs(mean=newmean, cov=sigmaHat, size=1)
        except ValueError:
            print "stop"
        alpha = D + 1
        beta = 1.0 / rhoSquare + np.dot(np.dot((means[m] - newXi[m]), inverseSigmam), (means[m] - newXi[m]))
        newTau[m] = gamma.rvs(alpha / 2.0, beta / 2.0, size=1)

    # gibbs sampling, so we accept
    return newXi, newTau, newEllInverse
...
def splitOrCombineTwoComponents(pi, means, lambdaInverse, A, Z, delta, X, tau, niu, rhoSquare, ellInverse, xi, r):
    ...
    if u < bk:
        return splitOneComponent(pi=pi, means=means, lambdaInverse=lambdaInverse, A=A, Z=Z, delta=delta, X=X, tau=tau,
                                 niu=niu, rhoSquare=rhoSquare, ellInverse=ellInverse, xi=xi, r=r)

    return combineTwoComponentsIntoOne(pi=pi, means=means, lambdaInverse=lambdaInverse, A=A, Z=Z, delta=delta, X=X,
                                       tau=tau, niu=niu, rhoSquare=rhoSquare, ellInverse=ellInverse, xi=xi, r=r)
...
def splitOneComponent(pi, means, lambdaInverse, A, Z, delta, X, tau, niu, rhoSquare, ellInverse, xi, r):
    ...
    newElementsInZj = []
    newElementsInZk = []
    sigmak = np.dot(np.dot(A, np.diag(1.0 / np.array(sorted(newlambdaInversek)))), A)
    sigmaj = np.dot(np.dot(A, np.diag(1.0 / np.array(sorted(newlambdaInversej)))), A)
    for nk in Z[k]:
        sigmak = np.dot(np.dot(A, np.diag(1.0 / np.array(sorted(newlambdaInversek)))), A)
        pnk = np.log(newPik) + np.log(np.linalg.det(sigmak)) + (
                    -1.0 / 2 * np.dot(np.dot((X[nk] - newMeank).T, np.linalg.inv(sigmak)), (X[nk] - newMeank)))

        sigmaj = np.dot(np.dot(A, np.diag(1.0 / np.array(sorted(newlambdaInversej)))), A)
        pnj = np.log(newPij) + np.log(np.linalg.det(sigmaj)) + (
                    -1.0 / 2 * np.dot(np.dot((X[nk] - newMeanj).T, np.linalg.inv(sigmaj)), (X[nk] - newMeanj)))

        p = pnk / (pnk + pnj)
        u = np.random.uniform(size=1)[0]
        if u < p:
            newElementsInZk.append(nk)
        else:
            newElementsInZj.append(nk)
    ...
    firstExponent = float(
        np.dot(np.dot((newMeanj - xij).reshape(1, -1), tauj * np.linalg.inv(sigmaj)), newMeanj - xij) +
    ...
    for d in range(0, D):
        ...
        numerator = lambdakd * np.power((newlambdajd * newelljd) / 2.0, -rj / 2.0) * np.power(
            (newlambdakd * newellkd) / 2.0, -rk / 2.0)
    ...
    oldCovariances = []
    for m in range(0, M):
        oldCovariances.append(np.dot(np.dot(A, np.diag(1.0 / np.array(sorted(lambdaInverse[m])))), A))
    oldCovariances = np.array(oldCovariances)

    newCovariances = []
    for m in range(0, len(newPi)):
        newCovariances.append(np.dot(np.dot(A, np.diag(1.0 / np.array(sorted(newLambdaInverse[m])))), A))
    ...
    R *= (np.power(2 * np.pi, - D / 2.0) * np.power(np.linalg.det(1.0 / tau[k] * sigmaoldk), 1.0 / 2) / (
        np.power(np.linalg.det(1.0 / tauj * 1.0 / tauk * np.dot(sigmaj, sigmak)), 1.0 / 2)))
    ...
    determinantOfJacobian = (np.power(pi[k], 3 * D + 1) / np.power(newPij * newPik,
                                                                   3 * D / 2.0)) * determinantOfJacobian
...
def combineTwoComponentsIntoOne(pi, means, lambdaInverse, A, Z, delta, X, tau, niu, rhoSquare, ellInverse, xi, r):
    ...
    firstExponent = float(
        np.dot(np.dot((means[i] - xi[i]).reshape(1, -1), tau[i] * np.linalg.inv(oldCovariances[i])), means[i] - xi[i]) +
        np.dot(np.dot((means[j] - xi[j]).reshape(1, -1), tau[j] * np.linalg.inv(oldCovariances[j])), means[j] - xi[j]) -
        np.dot(np.dot((newMeans[i] - newxi[i]).reshape(1, -1), newtau[i] * np.linalg.inv(newCovariances[i])),
               newMeans[i] - newxi[i]))
...
def birthOrDeathOfEmptyComponent(pi, means, lambdaInverse, Z, xi, tau, r, ellInverse, A, delta):
    ...
    if u < bn:  # birth of an empty component
        ...
        piProposal, meansProposal, lambdaInverseProposal, xiProposal, tauProposal, ellInverseProposal, rProposal, ZProposal, index = deathOfAComponent(
            pi=pi, means=means, lambdaInverse=lambdaInverse, Z=Z, xi=xi, tau=tau, r=r, ellInverse=ellInverse,
            delta=delta)
...
def adjustIndex(pi, means, lambdaInverse, xi, tau, ellInverse, r, Z):
    ...
    return pi[sortedIndex], means[sortedIndex], lambdaInverse[sortedIndex], xi[sortedIndex], tau[sortedIndex], \
           ellInverse[sortedIndex], r[sortedIndex], list(np.array(Z)[sortedIndex])


def computeMatrices(lambdaInverse, A):
    cov = []
    M = lambdaInverse.shape[0]
    for m in range(0, M):
...
def doMCMC(X, Mmax1, numberOfIterations=100000):
    N = len(X)
    rhoSquare = 0
    ...
    for i in range(0, numberOfIterations):
        pi, means, lambdaInverse, xi, tau, ellInverse, r, Z = adjustIndex(pi, means, lambdaInverse, xi, tau, ellInverse,
                                                                          r, Z)
        ...
        pi = updateMixingWeights(delta=delta, Z=Z, X=X, oldPi=pi, means=means, lambdaInverse=lambdaInverse)

        means, lambdaInverse = updateMeansAndCovariances(r=r, xi=xi, tau=tau, ellInverse=ellInverse, Z=Z, X=X,
                                                         means=means,
                                                         lambdaInverse=lambdaInverse, A=A, pi=pi)

        Z = updateAlocation(pi=pi, means=means, lambdaInverse=lambdaInverse, X=X, Z=Z)

        xi, tau, ellInverse = updateHyperparameters(r=r, zeta=zeta, xi=xi, tau=tau, niu=niu, ellInverse=ellInverse,
                                                    rhoSquare=rhoSquare,
                                                    means=means, lambdaInverse=lambdaInverse, A=A)

        pi, means, lambdaInverse, xi, tau, ellInverse, Z, r = splitOrCombineTwoComponents(pi=pi, means=means,
                                                                                          lambdaInverse=lambdaInverse,
                                                                                          A=A, Z=Z, delta=delta, X=X,
                                                                                          tau=tau, niu=niu,
                                                                                          rhoSquare=rhoSquare,
                                                                                          ellInverse=ellInverse,
                                                                                          xi=xi, r=r)

        pi, means, lambdaInverse, xi, tau, ellInverse, r, Z = birthOrDeathOfEmptyComponent(pi=pi, means=means,
                                                                                           lambdaInverse=lambdaInverse,
                                                                                           Z=Z, xi=xi, tau=tau, r=r,
                                                                                           ellInverse=ellInverse,
                                                                                           A=A, delta=delta)

    return Z, pi, means, computeMatrices(lambdaInverse, A)
